...
class AppStore extends Component {
  ...
  onEnteringInput = event => {
    this.setState({requiredInput: event.target.value})
  }
  ...
  filteringTabs = filteringApp => {
    const {activeTabId} = this.state
    const finalResult = filteringApp.filter(
      each => each.category === activeTabId,
    )
    return finalResult
  }

  filteringApp = requiredInput => {
    const filteredAppList = appsList.filter(each =>
      each.appName.toLowerCase().includes(requiredInput.toLowerCase()),
    )
    return filteredAppList
  }

  render() {
    const {requiredInput, activeTabId} = this.state
    const filteringApp = this.filteringApp(requiredInput)
    const finalResult = this.filteringTabs(filteringApp)
        ...
        <ul className="tab-container">
          {tabsList.map(eachTabItem => (
            <TabItem
              ...
              toChangeTabItems={this.toChangeTabs}
